...
async function swiggyScrape(query,location){
    ...
    const browser=await puppetter.lauch({
        headless:true,
    ...
    try{
         ...
         await delay(500)  ;// give time for tooltip to appear
     ...
     await page.evaluate(()=>{
        ...
        await page.evaluate(()=>{
            window.scrollBy(0,window.innerHeight) ; // small scroll to triggeer image laod 
        })
